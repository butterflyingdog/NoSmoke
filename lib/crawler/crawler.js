...
const ImageOCR = require('./imageocr').ImageOCR;
...
NSCrawler.prototype.crawl = function () {
  ...
  if (this.repeatingCrawlingCount >= maxRepeatCrawlingCount) {
    this.terminate('terminate due to maxRepeat');
    return;
  } else if (this.crawlingExpires) {
...
NSCrawler.prototype.explore = function(source) {
  ...
  source.value = hooks.filterSource(JSON.parse(source.value));

  node.checkDigest(this.config.platform, source, this).then(() => {

    /** 0. get currentNode ScreenSize from source */
    node.getScreenSize(source);
        ...
        if (this.currentNode.isFinishedBrowsing()) {
          ...
          if (this.currentNode.parent && this.currentNode.parent.type === 'tab') {
            if (this.currentNode.parent.isFinishedBrowsing()) {
              ...
              // console.log('tab ' + this.currentNode.digest + ' is finished, back ');
              this.back();
              return;
            } else {
            ...
            if (this.currentNode.depth === 0) {
              ...
              // console.log('normal ' +this.currentNode.digest +' is finished, back ');
              this.back();
            }
          }
        } else {
    ...
    if (node.depth >= this.config.testingDepth) {
      console.log('reach the max depth back ');
      this.back();
      return;
    }
    ...
    if (matches.length) {
      this.currentNode.actions = this.currentNode.produceNodeActions(matches, this.config);
      ...
      this.currentNode.actions = this.currentNode.produceNodeActions(elements, this.config);
      hooks.sortActionPriority(this.currentNode.actions, this);
    }
    ...
    /** 4. regex check */
    this.refreshScreen().then(()=>{
      if (this.currentNode.fileName && this.config.asserts && this.config.asserts.length > 0) {
        /** 4.1 check source by asserts */
        this.checkAssert(this.currentNode.raw);
        /** 4.2 check image by asserts */
        const reportPng = path.join(process.cwd(), 'reports', this.currentNode.fileName);
        fs.exists(reportPng, (isexist)=>{
        //  this.recursiveGetOcr(reportPng, this.currentNode.imagechecks);
          this.checkImages(reportPng);
        });
      }
    });

    /** 5. performAction on parent tab first */
    /* if (this.currentNode.parent && this.currentNode.parent.type === 'tab' && !this.currentNode.parent.isFinishedBrowsing()) {
      this.currentNode = this.currentNode.parent;
    }*/
...
NSCrawler.prototype.checkAssert = function (rawsource, imageaction) {
  for (let i = 0; i < this.config.asserts.length; i++) {
    if (this.checkContentMatch(rawsource, this.config.asserts[i].given, true)) {
      let isSuccess;
      if (this.config.asserts[i].then === undefined) {
        isSuccess = false;
      } else {
        isSuccess = true;
        if (this.checkContentMatch(rawsource, this.config.asserts[i].then, true)) {
      ...
      reporterTemplate.updateAssertion({
        'digest': this.currentNode.digest,
        'fileName': this.currentNode.fileName,
...
NSCrawler.prototype.back = function () {
  // perform current node's backAction
  let backAction = this.currentNode.backAction;
  if (backAction) {
    /** perform on backAction */
    console.log('click ' + this.currentNode.digest + ' backActon=' + JSON.stringify(backAction.source));
    this.excuteAction(backAction).then(() => {
      setTimeout(() => {
        this.refreshScreen();
        this.crawl();
  ...
  // perform alert confirm button
  let confirmAction ;
  for (let action of this.currentNode.actions) {
    if (this.checkContentMatch(action.source, this.config.targetElements.alertConfirm.searchValue, true)) {
      confirmAction = action;
      break;
    }
  }
  if (confirmAction) {
    /** perform on confirmAction */
    console.log('confirm action is ' + confirmAction.source.label);
    this.excuteAction(confirmAction).then(() => {
      setTimeout(() => {
        this.refreshScreen();
        this.crawl();
      }, this.config.newCommandTimeout * 1000);
  ...
  // perform webdriver back
  console.log(this.currentNode.digest + ' webdriver back');
...
NSCrawler.prototype.recursiveFilter = function (source, matches, exclusive) {
  ...
  */
  ...
  /** 4.find matched action, set specified value  */
  if (matches) {
    // Explicit mode
    for (let match in matches) {
      for (let tempsource in sourceArray) {
        if (this.checkContentMatch(sourceArray[tempsource], matches[match].searchValue, false)) {
          sourceArray[tempsource].input = matches[match].actionValue;
        }
      }
    }
  }
...
NSCrawler.prototype.performAction = function(actions) {
  ...
  return this.excuteAction(action);
};

NSCrawler.prototype.excuteAction = function(action) {
  ...
  return root.wdclient
    ...
    .then(() => {
      if (action.data.status === 0) {
        ...
        {
          let tag = action.source.label || action.source.value || action.source.name;
          let input = action.input ? action.input : '';
          console.log('before click ' + this.currentNode.digest + ' ' + tag + ', type=' + action.source.type + ', input=' + input + ', rect=' + JSON.stringify(action.source.rect));
        }
...
NSCrawler.prototype.checkImages = function (imagefile) {
  let that = this;
  gm(imagefile)
    .noProfile()
    .toBuffer('JPG', function(err, buffer) {
      if (!err) {
        imageocr.getocr(buffer)
          .then((ret)=>{that.checkAssert(ret);}, (error)=>{console.log('getocr error ' + error);});
      }
    });
};

/* depretate
NSCrawler.prototype.recursiveGetOcr = function (imagefile, imagechecks) {
  let multi = 1;
  let imageSuspending = imagechecks.pop();
  if (imageSuspending !== undefined) {
    let imagerect = imageSuspending.source.rect;
    let that = this;
    gm(imagefile)
      .size(function (err, size) {
        if (!err) {
          multi = size.width / that.currentNode.screenSize.maxX;
        }
      })
      .crop(imagerect.width * multi, imagerect.height * multi, imagerect.x * multi, imagerect.y * multi)
      .noProfile()
      .toBuffer('JPG', function(err, buffer) {
        if (err) {
          // return handle(err);
          that.recursiveGetOcr(imagefile, imagechecks);
        } else {
          imageocr.getocr(buffer)
            .then((ret)=>{
              that.checkAssert(ret, imageSuspending);
              that.recursiveGetOcr(imagefile, imagechecks);
            }, null);
        }
      });
...
NSCrawler.prototype.insertTabNode = function (rawElement) {
  if (!hooks.insertTabNode(rawElement, this)) {
    ...
    let node = new NSAppCrawlingTreeNode();
    node.actions = this.currentNode.produceNodeActions(rawElement, this.config);
    node.type = 'tab';
