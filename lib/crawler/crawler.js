...
NSCrawler.prototype.explore = function(source) {
  ...
  node.checkDigest(this.config.platform, source, this).then(() => {
    ...
    /**
    this.currentNode.actions.forEach( (value,index,arr)=>{
      console.log(this.currentNode.digest +' actions: ' + JSON.stringify(value.source.label) );
    });
    */
    ...
    this.refreshScreen().then(()=>{
      if (this.currentNode.fileName && this.config.asserts.length > 0) {
        ...
        fs.exists(reportPng, (isexist)=>{
          this.checkImages(reportPng, this.currentNode.imagechecks);
        });
      }
    });
...
NSCrawler.prototype.checkImages = function (imagefile, imagechecks) {
  // let i=0;
  let multi = 1;
  const ImageOCR = require('./imageocr').ImageOCR;
  const imageocr = new ImageOCR();
  let that = this;
  function recursiveGetOcr(i) {
    let check = imagechecks.pop();
    if (check !== undefined) {
      let imagerect = check.source.rect;
      gm(imagefile)
