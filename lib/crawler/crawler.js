  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    if (!candidate.isTriggered) {
      ...
      this.refreshScreen().then(()=>{
        if (this.currentNode.fileName) {
          const reportPng = path.join(process.cwd(), 'reports', this.currentNode.fileName);
          // console.log(this.currentNode.digest +', reportPng=' + reportPng );
          fs.exists(reportPng, (isexist)=>{
            checkImages(reportPng, this.currentNode.imagechecks);
          });
        }
      });
...
function checkImages(imagefile, imagechecks) {
  // let i=0;
  const multi = 3;
  ...
  function recursiveGetOcr(i) {
    ...
    if (check !== undefined) {
      ...
      // let cropfile = imagefile.replace('.', '-' + i + '.');
      gm(imagefile)
