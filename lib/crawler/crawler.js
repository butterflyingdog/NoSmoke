...
NSCrawler.prototype.performAction = function(actions) {
  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    if (!candidate.isTriggered) {
      ...
      // ignore  not hittable element
      if (candidate.source.isHittable === false) {
        continue;
      }
      ...
      let tag = candidate.source.label || candidate.source.value || candidate.source.name;
      let input = candidate.input ? candidate.input : '';
      console.log(' click ' + this.currentNode.digest + ' ' + tag + ', type=' + candidate.source.type + ', input=' + input + ', rect=' + JSON.stringify(candidate.source.rect) + ', xpath=' + candidate.location);
