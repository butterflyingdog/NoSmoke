...
NSCrawler.prototype.explore = function(source) {
  ...
  node.checkDigest(this.config.platform, source, this).then(() => {
    ...
    for (let i = 0; i < this.currentNode.actions.length; i++) {
      ...
      console.log(this.currentNode.digest + ' action=' + tag + ' type=' + action.type);
    }
    console.log('backAction = ' + JSON.stringify(this.currentNode.backAction));
...
NSCrawler.prototype.back = function () {
  ...
  if (backAction) {
    console.debug('backActon=' + JSON.stringify(backAction));
  ...
  if (matches) {
    ...
    for (let match in matches) {
      for (let tempsource in sourceArray) {
        if (this.checkContentMatch(tempsource, matches[match].searchValue, false)) {
          tempsource.input = matches[match].actionValue;
        }
      }
    }
  }
...
NSCrawler.prototype.performAction = function(actions) {
  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    if (!candidate.isTriggered) {
      ...
      console.log(this.currentNode.digest + ' click ' + tag + ', type=' + candidate.source.type + ', input=' + input + ', rect=' + JSON.stringify(candidate.source.rect));
