...
function NSAppCrawlingTreeNode() {
  ...
  this.imagechecks = []; // Units in {value : NSAppCrawlingTreeNodeAction}
  this.backAction = null;
  this.digest = null;
  this.reportSuite = null;
  this.repeatable = false;
  this.screenSize = {'maxX': 0, 'maxY': 0};
}
...
NSAppCrawlingTreeNode.prototype.getScreenSize = function(source) {
  const sourceValue = jsonpath.query(source, '$.value');
  const appRect = jsonpath.query(JSON.parse(sourceValue), '$.rect')[0];
  this.screenSize.maxX = appRect.width;
  this.screenSize.maxY = appRect.height;
};
...
NSAppCrawlingTreeNode.prototype.produceNodeActions = function(rawElements, config) {
  let actions = [];
  for (let index in rawElements) {
    ...
    if (action.source.isVisible && action.source.type === 'Image' && action.source.rect.width > 5 && action.source.rect.height > 5) {
      this.imagechecks.push(action);
    }

    // process hittable elements
    if (action.source.isHittable) {
      // find backaction element and others
      let action_tag = action.source.label || action.source.name || action.source.value || action.source.text;
      if (action_tag && action_tag === config.navigationBackKeyword[0]) {
        // find backaction element match navigationBackKeyword
        this.backAction = action;
        console.log(this.digest + ' backaction=' + this.backAction);
      } else if (action.location === config.backImage) {
        // find backaction element match backImage
        this.backAction = action;
        console.log(this.digest + ' backImagea=' + this.backAction);
      } else if (action.source.type === 'TextField' && action.source.TextField_count) {
        // ignore Text contain text element
      } else {
        actions.push(action);
      }
    }
