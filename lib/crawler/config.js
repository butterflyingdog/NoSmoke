...
NSCrawlerConfig.prototype.loadDefault = function() {
  ...
  let allConfigs = YAML.load(configFile);
  let crawlingConfig = allConfigs.crawlingConfig;
  ...
  // get backImageLoc
  const currentBundleId = allConfigs.desiredCapabilities.bundleId;
  const backImageConfigFile = path.join(__dirname, '..', '..', 'public', 'backImage.yml');
  const backImageConfig = YAML.load(backImageConfigFile).backImage;
  const backImageLoc = backImageConfig[currentBundleId];
  this.backImage = backImageLoc;
